...
use bkmr::application::services::factory::{create_bookmark_service, create_action_service};
...
fn given_stdin_content_when_add_command_with_stdin_flag_then_stores_content_in_url_column() {
    ...
    assert_eq!(bookmark.url, test_content, "URL should contain the stdin content");
    assert_eq!(bookmark.title, "test_stdin_command", "Title should match");
    assert_eq!(bookmark.description, "", "Description should be empty");
    assert!(bookmark.tags.contains(&Tag::new("_shell_").unwrap()), "Should have shell tag");
    assert!(bookmark.tags.contains(&Tag::new("test").unwrap()), "Should have test tag");
    assert!(bookmark.tags.contains(&Tag::new("stdin").unwrap()), "Should have stdin tag");
    
    // Verify it's stored correctly in the database
    let stored_bookmark = repository.get_by_id(bookmark.id.unwrap()).unwrap().unwrap();
    assert_eq!(stored_bookmark.url, test_content, "Stored URL should contain the stdin content");
}
...
fn given_add_command_with_stdin_and_type_shell_when_executed_then_creates_shell_bookmark() {
    ...
    // Assert
    assert!(bookmark.id.is_some(), "Bookmark should have an ID");
    assert_eq!(bookmark.url, test_script, "URL should contain the script content");
    assert_eq!(bookmark.title, title, "Title should match");
    assert_eq!(bookmark.description, "", "Description should be empty for stdin input");
    assert!(bookmark.tags.contains(&Tag::new("_shell_").unwrap()), "Should have shell tag");
    
    // Verify the bookmark type is correctly identified
    let stored_bookmark = repository.get_by_id(bookmark.id.unwrap()).unwrap().unwrap();
    assert_eq!(stored_bookmark.url, test_script, "Stored script should match input");
    assert!(stored_bookmark.tags.contains(&Tag::new("_shell_").unwrap()), "Should be tagged as shell");
}
...
fn given_stdin_with_multiline_content_when_add_command_then_preserves_formatting() {
    ...
    // Assert
    assert_eq!(bookmark.url, multiline_content, "Multiline content should be preserved exactly");
    
    // Verify in database
    let stored_bookmark = repository.get_by_id(bookmark.id.unwrap()).unwrap().unwrap();
    assert_eq!(stored_bookmark.url, multiline_content, "Stored multiline content should match");
    assert!(stored_bookmark.url.contains('\n'), "Should preserve newlines");
    assert!(stored_bookmark.url.contains("# This is a comment"), "Should preserve comments");
}