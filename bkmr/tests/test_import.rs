...
fn test_import_files_with_yaml_frontmatter() {
    ...
    let result = service.import_files(&paths, false, false, false);
...
"#).unwrap();
    ...
    if let Err(e) = result {
        assert!(e.to_string().contains("Duplicate name"), "Should be duplicate name error");
    }
}
...
"#).unwrap();
    ...
    let result = service.import_files(&paths, true, false, false);
...
fn test_import_files_dry_run() {
    ...
    let result = service.import_files(&paths, false, false, true);
    ...
    let file_bookmarks = all_bookmarks.iter()
